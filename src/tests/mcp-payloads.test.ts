...
import { buildCapturedMcpParameters } from "../modules/mcp-payloads.js";
  ...
  it("captures useful tool-call inputs without transport internals or duplicated intent", () => {
    const parameters = buildCapturedMcpParameters({
      ...
      params: {
        ...
        arguments: {
          context:
            "Review local project access before inspecting MCP analytics capture results.",
          projectId: 1,
  ...
  it("redacts PostHog tokens from captured string values", () => {
    const parameters = buildCapturedMcpParameters({
      ...
      params: {
        ...
        arguments: {
          summary:
            "Default project token api_token: phc_123456789012345678901234567890.",
        },
      },
    });
