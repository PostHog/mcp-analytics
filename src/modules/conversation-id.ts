...
import { DEFAULT_CONVERSATION_ID_DESCRIPTION } from "./constants.js";
...
export function addConversationIdToTool<
  TTool extends ConversationIdInjectableTool,
>(tool: TTool): TTool {
  ...
  if (schema?.oneOf || schema?.allOf || schema?.anyOf) {
    writeToLog(
      `WARN: Tool "${toolName}" has complex schema (oneOf/allOf/anyOf). Skipping conversation_id injection.`
    );
    return modifiedTool;
  }
...
export function addConversationIdToTools<
  TTool extends ConversationIdInjectableTool,
>(tools: TTool[]): TTool[] {
...
export function resolveConversationId(
  ...
  return { minted: true, conversationId: randomUUID() };
}
...
export function stripConversationId(args: unknown): unknown {
  if (
    !args ||
    typeof args !== "object" ||
    !(CONVERSATION_ID_PARAM_NAME in (args as Record<string, unknown>))
  ) {
    return args;
  }
  const { [CONVERSATION_ID_PARAM_NAME]: _omit, ...rest } = args as Record<
    string,
    unknown
  >;
  return rest;
...
export function injectConversationIdPromptBack(
  result: unknown,
  ...
  return {
    ...
    content: [
      ...resultObj.content,
      buildConversationIdPromptBack(conversationId),
    ],
  };
}
