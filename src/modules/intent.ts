...
async function runIntentFallback(
  ...
  try {
    const intent = normalizeIntent(
      await data.options.intentFallback(request, extra)
    );
    return intent ? { intent, source: "inferred" } : null;
  } catch (error) {
    writeToLog(`intentFallback callback error: ${error}`);
    return null;
  }
}
...
export function setEventIntent(
...
export function setExplicitContextIntent(
  event: UnredactedEvent,
  context: string
): void {
